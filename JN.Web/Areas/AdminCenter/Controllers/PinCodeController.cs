...
namespace JN.Web.Areas.AdminCenter.Controllers
{
    public class PinCodeController : BaseController
    {
        ...
        {
            ActMessage = "未使用活码列表";
            var list = PINCodeService.List(x => x.IsUsed == false).WhereDynamic(FormatQueryString(HttpUtility.ParseQueryString(Request.Url.Query)));
        ...
        public ActionResult Used(int? page)
        {
            ...
            var list = PINCodeService.List(x => x.IsUsed == true).WhereDynamic(FormatQueryString(HttpUtility.ParseQueryString(Request.Url.Query)));
            if (Request["IsExport"] == "1")
            {
                string FileName = string.Format("{0}_{1}_{2}_{3}", DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour, DateTime.Now.Minute);
                MvcCore.Extensions.ExcelHelperV2.ToExcel(list.ToList()).SaveToExcel(Server.MapPath("/Upload/" + FileName + ".xls"));
