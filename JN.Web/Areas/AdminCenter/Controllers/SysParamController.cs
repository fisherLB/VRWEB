...
namespace JN.Web.Areas.AdminCenter.Controllers
{
    ...
    public class SysParamController : BaseController
    {
        ...
        {
            int parentid = pid ?? SysParamService.List(x => x.PID == 0 && x.IsUse).OrderBy(x => x.ID).FirstOrDefault().ID;
        ...
        public ActionResult SysParam(FormCollection fc)
        {
            ...
            try
            {
                ...
                {
                    #region MyRegion
                    if (entity.ID == 3106 && entity.Value2 != "1")
                    {
                        return Json(new { result = "err", refMsg = "此参数不能手动关闭" }, JsonRequestBehavior.AllowGet);
                    }
                    #endregion
                    ...
                    if (entity.Value3Type == "scale" && (entity.Value3.ToDouble() < 0 || entity.Value3.ToDouble() > 1))
                        return Json(new { result = "err", refMsg = "参数值1为百分比，只能在0 ~ 1之间设定" }, JsonRequestBehavior.AllowGet);
                    if (entity.Value4Type == "scale" && (entity.Value4.ToDouble() < 0 || entity.Value4.ToDouble() > 1))
                        return Json(new { result = "err", refMsg = "参数值1为百分比，只能在0 ~ 1之间设定" }, JsonRequestBehavior.AllowGet);
                ...
                SysDBTool.Commit();
                Services.Manager.CacheHelp.ClearSysParamsCache();
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Modify(int id, int? pid)
        {
            ...
            if (model.ID == 0)
            {
                int thepid = pid ?? 1000;
                model.PID = thepid;
                model.Sort = SysParamService.List(x => x.PID == thepid).Count() > 0 ? SysParamService.List(x => x.PID == thepid).Max(x => x.Sort) + 1 : 1;
            }
        ...
        public ActionResult SysSetting(FormCollection fc)
        {
            ...
            string ImgCount = fc["ImgCount"];
            if (!string.IsNullOrEmpty(ImgCount))
            {
                int iImgCount = ImgCount.ToInt();
                string ImgAlt = "";
                ...
                ImgAlt = ImgAlt.TrimEnd(';');
                if (string.IsNullOrEmpty(ImgAlt))
                {
                    ViewBag.ErrorMsg = "请输入链接！";
                    return View("Error");
                }
                entity.BannerAlt = ImgAlt;
                entity.BannerPic = entity.BannerPic.TrimStart(';');
            }
            string appImgCount = fc["appImgCount"];
            if (!string.IsNullOrEmpty(appImgCount))
            {
                int iAppImgCount = appImgCount.ToInt();
                string ImgAlt = "";
                for (int i = 0; i < iAppImgCount; i++)
                {
                    ImgAlt += fc["applink" + i] + ";";
                }
                ...
                entity.AppBannerAlt = ImgAlt;
                entity.AppBannerPic = entity.AppBannerPic.TrimStart(';');
            }
            ...
            SysDBTool.Commit();
            Services.Manager.CacheHelp.ClearSysSetCache();
            ...
            string IphoneAppUrl = fc["IphoneAppUrl"];//苹果下载
            string AndroidAppUrl = fc["AndroidAppUrl"];//安卓下载
            ...
            if (ModifyWebConfig.ToBool())
            {
                ...
                ConfigHelper.UpdatetConfig("IphoneAppUrl", IphoneAppUrl);
                ConfigHelper.UpdatetConfig("AndroidAppUrl", AndroidAppUrl);
            }
        ...
        public ActionResult Language(string sv, int? pid, int? page)
        {
            ...
            if (param != null)
            {
                var list = LanguageService.List(x => x.LanguageName == param.Value2).WhereDynamic(FormatQueryString(HttpUtility.ParseQueryString(Request.Url.Query)));
                if (!string.IsNullOrEmpty(sv))
                    list.Where(x => string.IsNullOrEmpty(x.Value));
                return View(list.OrderByDescending(x => x.Location).ToPagedList(page ?? 1, 15));
            }
        ...
        public ActionResult MakeLangXML()
        {
            ...
            foreach (Data.SysParam param in paramList)
            {
                ...
                //去重
                foreach (var item in langList.OrderBy(x => x.Name).GroupBy(x => x.Name).Where(x => x.Count() > 1))
                {
                    foreach (var repeatitem in item)
                    {
                        if (item.ToList().FindIndex(x => x.ID == repeatitem.ID) == 0) continue;
                        LanguageService.Delete(repeatitem.ID);
                        langList.Remove(repeatitem);
                    }
                }
                LanguageService.Commit();
        ...
        public Hashtable UpPic1(string sid)
        {
            ...
            try
            {
                string oldLogo = "/Upload/SysSetting/" + sid + "/";
