...
namespace JN.Services.Tool
{
    public class UploadPic : System.Web.UI.Page
    {
        ...
        /// <param name="maxLength">允许上传的最大大小，以M为单位</param>
        /// <param name="savePath">保存文件的目录，注意是绝对路径,如：Server.MapPath("~/upload/");</param>
        /// <param name="fileName">标识文件特有的功能放前缀 如Pay_UserName</param>
        public static string MvcUpload(HttpPostedFileBase myFileUpload, string[] allowExtensions, int maxLength, string savePath, string fileName = "")
        {
            // 文件格式是否允许上传
            bool fileAllow = false;
            ...
            if (myFileUpload != null)
            {
                ...
                if (fileAllow)
                {
                    ...
                    if (!JN.Services.Tool.FileValidation.IsImage(path))
                    {
                        System.IO.File.Delete(path); //删除原文件
                        throw new Exception("您上传的不是图片，如果你上传多次不是图片的文件，系统将识别为木马文件，系统将自动冻结您的账号");
                    }

                    //判断是否是图片，排除木马 (判断文件头，文件流)
                    if (!JN.Services.Tool.FileValidation.IsPicture(path))
                    {
                        System.IO.File.Delete(path); //删除原文件
                        throw new Exception("您上传的不是图片，如果你上传多次不是图片的文件，系统将识别为木马文件，系统将自动冻结您的账号");
                    }
