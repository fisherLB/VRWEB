...
namespace JN.Services.Tool
{
    public class IOHelper
    {
        ...
        /// <summary>复制文件到指定目录,并赋予新名字
        /// 
        /// </summary>
        /// <param name="orignFile">原始路径(相对路径)</param>
        /// <param name="newFile">新目录（相对路径）</param>
        /// <returns>新文件名</returns>
        public static string FileCopyNewName(string orignFile, string newFile)
        {
            orignFile = System.Web.HttpContext.Current.Server.MapPath(orignFile);
            newFile = System.Web.HttpContext.Current.Server.MapPath(newFile);
            try
            {
                ...
                string fileExtension = System.IO.Path.GetExtension(orignFile).ToLower();
                string saveName = Guid.NewGuid() + fileExtension;
                File.Copy(orignFile, newFile + saveName, true);
                return saveName;
            }
            catch (Exception)
            {
