...
namespace JN.Services.Tool
{
    public class StringHelp
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetPrice(string url,string roleid, string cname, bool b = false)
        {
            try
            {
                string ja1a = "0";
                string key = "Price_" + roleid;//币种id
                if (MvcCore.Extensions.CacheExtensions.CheckCache(key) && MvcCore.Extensions.CacheExtensions.GetCache<string>(key) != "0")
                {
                    ja1a = MvcCore.Extensions.CacheExtensions.GetCache<string>(key);
                }                
                else
                {
                    ...
                    ja1a = ja[0]["price_" + cname.ToLower()].ToString();

                    //或者  
                    //ja1a = "2";

                    MvcCore.Extensions.CacheExtensions.SetCache(key, ja1a, MvcCore.Extensions.CacheTimeType.ByMinutes, 10);//10分钟更新一次
                }
                return ja1a;
            }
            catch (Exception webEx)
            {
                if (b)
                {
                    JN.Services.Manager.logs.WriteErrorLog(webEx);
                }
        ...
        {
            Regex objPhone = new Regex("^0?(13|14|15|16|17|18|19)[0-9]{9}$");
        ...
        private static readonly string[] StrChineseCharList = 
            { 
                ...
                "L狫窂哰唠崂浶勞痨铹僗嶗憥朥癆磱簩蟧醪鐒顟髝老佬咾姥恅荖栳珯硓铑蛯銠潦橑鮱轑涝烙嗠耢酪嫪憦澇橯耮軂仂阞乐叻忇扐氻艻玏泐竻砳勒楽韷簕鳓鰳饹餎雷嫘缧蔂樏畾檑縲盾櫑瓃羸礧"   , 
        ...
        #region 获取Json对象
        /// <summary>
        /// 验证用于身份验证的远程安全套接字层 (SSL) 证书。（重写，总是允许）
        /// </summary>
        ...
        /// <summary>
        /// 通过URL获取Json对象，支持HTTPS，默认使用GET方法
        /// </summary>
        /// <param name="url">第三方请求地址</param>
        /// <param name="parameters">请求参数，格式参数1=值1&参数2=值2 示例 "a=1&b=3"</param>
        /// <param name="method">请求方式（POST或GET）,post方法必须使用parameters参数传递请求参数</param>
        /// <returns></returns>
        public static JObject RequestJson(string url, string parameters = null, string method = "GET")
        {
            try
            {
                ...
                if (parameters != null && parameters != "" && method.ToLower() == "post")
                {
                    ...
                    catch//请求失败时尝试切换请求方法再次请求
                    {
                        if (parameters != null && parameters != "")
                            url += "?" + parameters;
                        return RequestJson(url);//使用GET方法请求
                    }
                }
                HttpWebResponse response = request.GetResponse() as HttpWebResponse;
                Stream streams = response.GetResponseStream();   //获取响应的字符串流  
                StreamReader sr = new StreamReader(streams); //创建一个stream读取流  
                string html = sr.ReadToEnd();   //从头读到尾，放到字符串html  
                return html == "" ? null : JObject.Parse(html);
            }
            catch (Exception webEx)
            {
        ...
        public static string UserNameHid(string s)
        {
            ...
            else
            {
                ...
                return s.Substring(0, 1) + start + s.Substring(s.Length - 1);
            }   
        }
        ...
        public static string BankStr(string s)
        {
            if (s.Length < 18) return s;
            return s.Substring(0, 6) + "********" + s.Substring(14);
        }
        ...
        {
            if (errmsg.Contains("Error when updating entries"))//更新条目时出错
                return "The network system is busy, please try again later!";
            else
                return errmsg;
        }

        #region 获取首个以分号区隔的字符串
        /// <summary>
        /// 获取首个以分号区隔的字符串
        /// </summary>
        ...
        public static string GetFirstStr(string str)
        {
            ...
            foreach (string item in strImgs)
            {
                if (item.Length == 0)
                    continue;
                returnStr = item;
        ...
        #region 验证支付方式
        public static bool ChkPayWay(string payWay)
        {
            bool fag = false;
            switch (payWay)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public static string IPGetCity(string ip)
        {
            try
            {     
                WebClient MyWebClient = new WebClient();
                ...
                dynamic dic = JN.Services.Tool.JsonConverter.ConvertJson(pageHtml);
                if (dic.code == 0)
                {
                    return dic.data.region + dic.data.city;
                }
                else {
                    return "";
                }
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static string FilterSqlHtml(string str)
        {
            ...
            str = Regex.Replace(str, @"<html[^>]*?>.*?<ml>", "", RegexOptions.IgnoreCase);
            ...
            str = Regex.Replace(str, @"<link[^>]*?>.*?<nk>", "", RegexOptions.IgnoreCase);
            ...
            str = Regex.Replace(str, @"<", string.Empty);
            str = Regex.Replace(str, @">", string.Empty);
            str = Regex.Replace(str, @"http://", "", RegexOptions.IgnoreCase);
            str = Regex.Replace(str, @"https://", "", RegexOptions.IgnoreCase);
            str = Regex.Replace(str, @"update", "", RegexOptions.IgnoreCase);
            str = Regex.Replace(str, @"create", "", RegexOptions.IgnoreCase);
            str = Regex.Replace(str, @"<script", "", RegexOptions.IgnoreCase);
            //http：//过滤字符串
            str = Regex.Replace(str, @"^(http|https|ftp)\://|[a-zA-Z0-9\-\.]+\.[a-zA-Z]{2,3}(:[a-zA-Z0-9]*)?/?([a-zA-Z0-9\-\._\?\,\'/\\\+&$%\$#\=~])*$", "", RegexOptions.IgnoreCase);
            return str;
        }
        #endregion
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetCurInfo(string url, string cname)
        {
            try
            {
                WebClient MyWebClient = new WebClient();
                ...
                string pageHtml = Encoding.Default.GetString(pageData);
                JArray ja = (JArray)JsonConvert.DeserializeObject(pageHtml);
                string into = ja[0]["price_" + cname.ToLower()].ToString() + "|" + ja[0]["24h_volume_"+cname.ToLower().ToString()].ToString();
                //或者  
        ...
        /// </summary>
        /// <returns></returns>
        public static JArray GetCurInfo(string url)
        {
            try
            {
                WebClient MyWebClient = new WebClient();
        ...
        // Json->Object
        public static Data Deserializes(string json)
        {
            JavaScriptSerializer jsonSerializer = new JavaScriptSerializer();
            //执行反序列化
