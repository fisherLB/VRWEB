...
namespace JN.Services.Manager
{
    ...
    public partial class logs
    {
        ...
        public static void WindowsServiceWriteLog(string readme)
        {
            ...
            string path = dir + DateTime.Now.ToString("yyyy-MM-dd") + "WServ_InnPointLog.txt";

            StreamWriter FileWriter = new StreamWriter(path, true, System.Text.Encoding.UTF8); //创建日志文件
            FileWriter.Write(System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + " >>" + readme + "\r\n");
            FileWriter.Close(); //关闭StreamWriter对象
        }
        ...
        public static void WriteGiveLog(string msg)
        {
            try
            {
                if (!string.IsNullOrEmpty(msg))
                {
                    ...
                    Page page = new Page();
                    HttpServerUtility server = page.Server;
                    string dir = server.MapPath("/App_Data/Log/Give/");
                    if (!Directory.Exists(dir))
                    {
                        Directory.CreateDirectory(dir);
                    }
        ...
        //{
        //    public static string GetAllMessage(this Exception ex, string beforStr = "")
        //    {
        //        string message = beforStr + (string.IsNullOrEmpty(beforStr) ? "" : "└") + ex.Message;

        //        if (ex.GetType().Name == "AggregateException")
        //        {
        //            if (((AggregateException)ex).InnerExceptions != null && ((AggregateException)ex).InnerExceptions.Count > 0)
        //            {
        //                foreach (var item in ((AggregateException)ex).InnerExceptions)
        //                {
        //                    message += "\r\n" + item.GetAllMessage(beforStr + "    ");
        //                }
        //            }
        //        }
        //        else
        ...
        //            {
        //                message += "\r\n" + ex.InnerException.GetAllMessage(beforStr + "    ");
        //            }
        //        }
        //        return message;
        //    }
